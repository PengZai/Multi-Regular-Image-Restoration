...
%%
%repmat copy matirx
%J and I represent horizontal and vertical direction, respectively.
...
%conj(1+3i) = 1-3i
%Dh2 is modulus (a+bi)(a-bi) = a^2+b^2
%Dh2 = FFT(Dh^T*Dh)
Dh2=Dh_FFT.*conj(Dh_FFT);
Dv2=Dv_FFT.*conj(Dv_FFT);

%% initilation
% compute gradient of y, for instance, DyY(i,j) = Y(i,j) - Y(i-1,j)
%dx horizontal  Dhy
%dy vertical Dvy
[dx,dy]=BackwardD(y);
...
AsDhy=HC_FFT.*fft2(dx);%FFT(A^T{D_h{y}})
AsDvy=HC_FFT.*fft2(dy);%FFT(A^T{D_v{y}})

ATA_FFT=H_FFT.*HC_FFT; %FFT(A^T*A)
...
for i=1:maxIter
    ...
    %% f
    if method==1
        %soft-thresholding operator “Ï–‘
        [fx,fy] = anisoTVDenoise(miu/delta2,dx+qx,dy+qy);
       
    else
        %group shrinkage operator Õ¨–‘
...
%         imshow(x+mean_y);pause();
    ...
    normdx=norm(dxd(:));
    normdy=norm(dyd(:));
    fprintf('iter = %d, norm d = %f\n', i, normdx/norm(dxp(:))+ normdy/norm(dyp(:)));
...
%% nested functions
%BackwardD: backward difference
    function [Dux,Duy] = BackwardD(U)
        % Backward finite difference operator
        %difference and add margin difference
        %shape of diff(U,1, 2) is 512*511 double, where Dux = horizontal col
        %shape of diff(U,1, 1) is 512*511 doubel, where Duy = vertical row
        Dux = [ U(:,1) - U(:,end),diff(U,1,2)];
        Duy = [ U(1,:) - U(end,:);diff(U,1,1)];
    end

   % Backward finite difference operator(conjugate)
    ...
    function DtXY = div(X,Y)
        %divergence of gradient x = laplace x = DvDvX + DhDhX
